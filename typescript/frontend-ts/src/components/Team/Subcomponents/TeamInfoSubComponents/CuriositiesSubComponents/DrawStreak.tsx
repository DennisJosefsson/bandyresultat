...
const DrawStreak = ({ team }: { team: SingleTeam }) => {
  if (
    !team.drawStreak ||
    team.drawStreak.filter((streak) => streak.game_count > 2).length === 0
  )
    return null

  const drawStreak = team.drawStreak.filter((streak) => streak.game_count > 2)
