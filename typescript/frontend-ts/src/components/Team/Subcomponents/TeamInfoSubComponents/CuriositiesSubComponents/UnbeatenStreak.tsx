...
const UnbeatenStreak = ({ team }: { team: SingleTeam }) => {
  if (
    !team.unbeatenStreak ||
    team.unbeatenStreak.filter((streak) => streak.game_count > 5).length === 0
  )
    return null
  const unbeatenStreak = team.unbeatenStreak.filter(
